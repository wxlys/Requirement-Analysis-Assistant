...
3. 读取 `test_case_generation/prompt.md`，生成 `{workspace}/test_case_generation/test_cases.json`。
4. 执行：
...
5. 如果校验失败，读取 `{workspace}/test_case_generation/reports/validation.json`，结合 `test_case_generation/repair_prompt.md` 修复 `{workspace}/test_case_generation/test_cases.json`，然后重新校验。
